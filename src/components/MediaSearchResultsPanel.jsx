...
export default function MediaSearchResultsPanel({
  query,
  mediaType,
  apiBaseUrl,
  accentColor,
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${apiBaseUrl}/api/v1/media/search?q=${encodeURIComponent(trimmed)}&type=${type}&limit=8`,
  ...
  }, [apiBaseUrl, mediaType, query, suspend]);
    ...
    >
      {query.trim().length < 2 && (
        <div style={{ padding: "18px 16px", fontSize: 13, lineHeight: 1.6, color: "rgba(245,240,235,0.72)" }}>
          제목을 2글자 이상 입력하면 이 영역에 검색 결과가 표시됩니다.
        </div>
      )}
          ...
          >
            {item.poster_url ? (
              ...
              <span style={{ fontSize: 10, color: accentColor }}>
                {mediaType === "게임" ? "GAME" : mediaType === "시리즈" ? "TV" : "MOVIE"}
              </span>
            )}
          </div>
